...
static func calculate_damage(skill: Dictionary, attacker_stats: Dictionary, defender: Dictionary) -> DamageResult:
	...
	var scaled_damage = base_damage * (1.0 + stat_value * 0.1)
	...
	var crit_chance = _get_stat_value(attacker_stats, "dexterity") * 0.05  # 5% per dexterity
	result.is_critical = randf() < crit_chance
	var crit_multiplier = 1.5 if result.is_critical else 1.0
...
static func calculate_basic_attack(attacker_stats: Dictionary, defender: Dictionary) -> DamageResult:
	...
	return calculate_damage(basic_attack_skill, attacker_stats, defender)
...
static func _calculate_defense(defender: Dictionary) -> float:
	...
	return vigor * 2.0  # 2 defense per vigor
...
static func calculate_derived_stats(base_stats: Dictionary) -> Dictionary:
	...
	return {
		...
		"max_mp": resonance * 5,
		"defense": vigor * 2,
		"crit_rate": dexterity * 0.05,  # 5% per dexterity
		"speed": agility * 2,  # For initiative calculations
	}
